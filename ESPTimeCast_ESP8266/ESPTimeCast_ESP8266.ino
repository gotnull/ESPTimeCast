...
#define DATA_PIN 2 // D4 (DIN)
#define CS_PIN 15  // D8 (CS)
#define CLK_PIN 4  // D2 (CLK)
...
char ssid[32] = "PANDORIA";
char password[32] = "Y4sgzz27ox";
char openWeatherApiKey[64] = "c0a3e9da669747436668700ed1a8f07f";
char openWeatherCity[64] = "Melbourne";
char openWeatherCountry[64] = "AU";
char weatherUnits[12] = "metric";
char timeZone[64] = "Australia/Melbourne";
char language[8] = "en";
String mainDesc = "";
String detailedDesc = "";
...
void connectWiFi()
{
  ...
  if (!credentialsExist)
  {
    Serial.println(F("[WIFI] No saved credentials. Bypassing SmartConfig for diagnostic."));
    // Temporarily force AP mode for testing web server functionality
    WiFi.mode(WIFI_AP);
    WiFi.softAP(AP_SSID, DEFAULT_AP_PASSWORD);
    isAPMode = true;
    Serial.print(F("[WIFI] Forced AP Mode. IP address: "));
    Serial.println(WiFi.softAPIP());

    // Show AP info on display
    pendingIpToShow = WiFi.softAPIP().toString();
    showingIp = true;
    ipDisplayCount = 0;
    P.displayClear();
    P.setCharSpacing(1);
  ...
  while (animating)
  {
    ...
    {
      Serial.println(F("[WiFi] Connection failed. Clearing invalid credentials and staying in STA mode for provisioning."));

      // Clear invalid credentials but stay in STA mode
      ...
      // Show setup needed message on display
      pendingIpToShow = "STARTING...";
      showingIp = true;
      ipDisplayCount = 0;
      P.displayClear();
      P.setCharSpacing(1);
      ...
      Serial.println(F("[WIFI] Device ready for provisioning via mobile app."));
      animating = false;
      break;
    }
...
void saveWiFiCredentialsToConfig(const char *newSSID, const char *newPassword)
{
  DynamicJsonDocument doc(2048);
  ...
  if (!f)
  {
    Serial.println(F("[SECURITY] ERROR: Cannot write to /config.json to save credentials!"));
    return;
  }
  serializeJson(doc, f);
  f.close();
  Serial.println(F("[SECURITY] Saved WiFi credentials to config.json."));
}
...
void setupWebServer()
{
  ...
  // Wi-Fi scan endpoint for network discovery
  server.on("/scan_networks", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    Serial.println(F("[WEBSERVER] Request: /scan_networks"));
    ...
    for (int i = 0; i < networks; i++) {
      ...
      json += "\"encryption\":" + String((WiFi.encryptionType(i) == ENC_TYPE_NONE) ? 0 : 1);
      json += "}";
    }
    json += "]";
  ...
  // SmartConfig provisioning endpoint
  server.on("/start_smartconfig", HTTP_POST, [](AsyncWebServerRequest *request)
            {
    Serial.println(F("[WEBSERVER] Request: /start_smartconfig"));

    WiFi.mode(WIFI_STA);
    WiFi.beginSmartConfig();

    request->send(200, "application/json", "{\"status\":\"SmartConfig started\"}"); });

  // Check SmartConfig status
  server.on("/smartconfig_status", HTTP_GET, [](AsyncWebServerRequest *request)
            {
    bool smartConfigDone = WiFi.smartConfigDone();
    String json = "{\"done\":" + String(smartConfigDone ? "true" : "false");
    if (smartConfigDone) {
      json += ",\"ssid\":\"" + WiFi.SSID() + "\"";
    }
    json += "}";
...
void setup()
{
  ...
  else if (WiFi.status() == WL_CONNECTED)
  {
    ...
    setupTime(); // Only setup NTP when WiFi is connected
  }
  else
  {
  ...
  if (displayMode == 0)
  {
    ...
    else if (!ntpSyncSuccessful)
    {
      ...
      if (!ntpSyncSuccessful && !weatherAvailable)
      {
        ...
        String errorMsg = showNtpError ? "NO TIME" : "NO WEATHER";
        textEffect_t actualScrollDirection = getEffectiveScrollDirection(PA_SCROLL_LEFT, flipDisplay);
        P.displayScroll(errorMsg.c_str(), PA_CENTER, actualScrollDirection, 50);
      }
      else if (!ntpSyncSuccessful)
      {
        textEffect_t actualScrollDirection = getEffectiveScrollDirection(PA_SCROLL_LEFT, flipDisplay);
        P.displayScroll("NO TIME", PA_CENTER, actualScrollDirection, 50);
      }
      else if (!weatherAvailable)
      {
        textEffect_t actualScrollDirection = getEffectiveScrollDirection(PA_SCROLL_LEFT, flipDisplay);
        P.displayScroll("NO WEATHER", PA_CENTER, actualScrollDirection, 50);
      }
    }
    // --- DISPLAY CLOCK ---
    else
